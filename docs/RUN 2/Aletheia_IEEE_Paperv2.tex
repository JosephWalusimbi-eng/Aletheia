...
\author{Joseph Walusimbi,~\IEEEmembership{}
  ...
  and~Precious Nasasara~\IEEEmembership{}
...
\thanks{P. Nasasara is with the School of Health Sciences,
Soroti University, Soroti, Uganda
(e-mail: nasasirap@gmail.com).}}
...
prevalence in East Africa. The system runs entirely on commodity
laptop hardware (Intel Core i5, 8\,GB DDR4 RAM) without internet
connectivity, occupying only 1.93\,GB of storage in its primary
deployment format. Evaluation demonstrates a Top-1 diagnostic
accuracy of 70.0\%, Top-3 accuracy of 100.0\%, BERTScore-F1 of
0.909, and METEOR of 0.467 across ten representative clinical case
categories. The system achieves an Expected Calibration Error (ECE)
of 0.185 and passes the Africa Deep Tech Challenge 2026 (ADTC 2026)
memory budget constraint of 7\,168\,MB, achieving
a peak inference RAM of approximately 3\,730\,MB on the standardised
...
differential diagnosis, GGUF quantisation, East Africa, sub-Saharan Africa, Africa.
\end{IEEEkeywords}
...
The Aletheia training dataset was constructed from two sources:
...
The system is designed to be invoked through a lightweight interface
layer (terminal, Python wrapper, or future GUI) that accepts
structured clinical input and returns structured JSON containing
ranked differentials, recommended tests, rationale, and red flags.
...
\caption{Top-1 (70.0\%) and Top-3 (90.0\%) diagnostic accuracy
...
of 90.0\%, meaning the correct diagnosis is almost always present
...
\caption{Left: Reliability diagram with ECE~=~0.185.
...
Estimated peak RAM        & $\sim$3{,}730\,MB & $\sim$3{,}075\,MB \\
ADTC ceiling (7{,}168\,MB)& \textbf{PASS} & \textbf{PASS} \\
Margin below ceiling      & 3{,}438\,MB & 4{,}093\,MB \\
...
\begin{enumerate}
  \item \textbf{Synthetic training data}: The majority of training
        ...
        templates. While carefully designed to reflect East African
        clinical presentation patterns, this data has not been
        validated by practising Ugandan clinicians, and the
        probability distributions assigned to differentials reflect
        the author's clinical knowledge synthesis rather than
        empirical epidemiological data.

  \item \textbf{Evaluation case set size}: The quantitative
        evaluation was conducted on 10 core case categories. A
        larger and clinician-validated evaluation set is needed to
        robustly estimate real-world performance.
  ...
  \item \textbf{No prospective clinical validation}: Aletheia has
        not yet been evaluated in a prospective clinical study with
        real patients and practising clinicians. Such a study is
...
\begin{itemize}
  \item Prospective clinical validation study across multiple
        health facilities in Eastern Uganda --- including district
        hospitals, regional referral hospitals, and primary health
        centres --- involving 50+ clinical officers and 500+ patient
        cases, subject to IRB approval from Soroti University and
        the Uganda National Council for Science and Technology (UNCST);
  \item Expansion of clinical conditions to include obstetric
        emergencies, paediatric dosing, snake envenomation, and
        trauma triage;
  \item Integration of a lightweight desktop GUI for non-technical
        clinical users;
  \item Clinician feedback loop for continuous model improvement
        and calibration;
  \item Submission to the Uganda National Drug Authority (NDA)
        software-as-a-medical-device regulatory pathway;
  \item Kiswahili and Ateso language support.
\end{itemize}
...
BERTScore-F1 of 0.906 while operating entirely within the memory
...
a system that presents clinically relevant differential diagnoses
in 90\% of cases --- running on hardware already present in most
...
health use licence. The authors invite collaboration from Ugandan
...
Hospital, Soroti District, whose daily practice under severe resource
constraints identified the problem this work addresses. Training compute was provided
through Google Colab Pro. The author acknowledges the Africa Deep
...
\textit{Conflict of Interest}: J.~Walusimbi is the founder and
director of Arapai Technologies International Limited, a technology
company based in Uganda. Aletheia is intended for future
commercialisation through this entity. The remaining authors declare
