...
The system provides two interfaces for clinical use: a terminal CLI for scripting and integration, and a web-based graphical user interface (GUI) \cite{aletheia_github} served by the Python standard library, which runs locally in the browser without internet connectivity and requires no third-party web framework. Both interfaces accept structured clinical input and return structured JSON containing ranked differentials, recommended tests, rationale, and red flags.
...
\begin{itemize}
  ...
  \item Development of a lightweight desktop application to
        complement the existing web UI and terminal CLI,
